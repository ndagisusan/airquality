...
void setup() {
  ...
  Serial.println(" ");
  delay(60000); // 60s(1 min) for stabilization, of the SDS011 and the MQ sensors
}

void loop() {
  tm = millis()-60000;  //No of milliseconds that have passed since the program began less the stabilization delay time
  read(); 
...
void read(){
  ...
  //----------------------SDS 011-------------------------
  int pm25High = pulseIn(pm25Pin, HIGH, timeoutDelay)/1000;   //Convert from microseconds to milliseconds
  ...
  Serial.print(mq9Value/55);  //After callibration
  Serial.println("ppm");
  Serial.print("CO2 concentration: ");
  Serial.print(mq135Value/0.06);  //After callibration
  Serial.println("ppm");  
  ...
  //--------------------------JSON FORMAT-------------------------------
  String ts = format<unsigned long>("ts",tm/1000);  //Convert to seconds
  String pm25 = format<int>("pm25",pm25High-2);
  String pm10 = format<int>("pm10",pm10High-2);
  String co = format<int>("co",mq9Value/55);  //After callibration
  String co2 = format<int>("co2",mq135Value/0.06);  //After callibration
